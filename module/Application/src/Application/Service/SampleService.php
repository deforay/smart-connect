...
class SampleService
{
    ...
    public function uploadSampleResultFile($params)
    {
        ...
        if (!file_exists(UPLOAD_PATH . DIRECTORY_SEPARATOR . "vl-sample-result" . DIRECTORY_SEPARATOR . $fileName)) {
            if (move_uploaded_file($_FILES['importFile']['tmp_name'], UPLOAD_PATH . DIRECTORY_SEPARATOR . "vl-sample-result" . DIRECTORY_SEPARATOR . $fileName)) {
                $objPHPExcel = \PhpOffice\PhpSpreadsheet\IOFactory::load(UPLOAD_PATH . DIRECTORY_SEPARATOR . "vl-sample-result" . DIRECTORY_SEPARATOR . $fileName);
    ...
    public function saveFileFromVlsmAPI(){
        ...
        if (!file_exists(UPLOAD_PATH . DIRECTORY_SEPARATOR . "vlsm") && !is_dir(UPLOAD_PATH . DIRECTORY_SEPARATOR . "vlsm")) {
            mkdir(UPLOAD_PATH . DIRECTORY_SEPARATOR . "vlsm",0777);
        }

        if (!file_exists(UPLOAD_PATH . DIRECTORY_SEPARATOR . "vlsm" . DIRECTORY_SEPARATOR . $fileName)) {
            if (move_uploaded_file($_FILES['vlFile']['tmp_name'], UPLOAD_PATH . DIRECTORY_SEPARATOR . "vlsm" . DIRECTORY_SEPARATOR . $fileName)) {
                $apiData = (array)json_decode(file_get_contents(UPLOAD_PATH . DIRECTORY_SEPARATOR . "vlsm" . DIRECTORY_SEPARATOR . $fileName));
            }
        }
        // Debug::dump($apiData['data']);die;
        if(count($apiData['data']) > 0){
            $apiDetails = (array)$apiData['data'];
            foreach($apiDetails as $key=>$row){
            ...
            //remove directory
            $common->removeDirectory(UPLOAD_PATH . DIRECTORY_SEPARATOR . "vlsm" . DIRECTORY_SEPARATOR . $fileName);
        }
        return array(
            'status'    => 'success',
